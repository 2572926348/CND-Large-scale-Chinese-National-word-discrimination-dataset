...
from util import load_pkl_data, save_pkl_data, EarlyStopping
from model import BertForClozeBaseline
...
class config:
    ...
    DO_TRAIN = True
    DO_TEST = True
    BERT_PATH = project_dir + "/ChineseBERT-base"
...
class ClozeDataset:
    ...
    def __getitem__(self, item):
        ...
        # Return the processed chid_data where the lists are converted to `torch.tensor`s
        return {
...
def train_fn(data_loader, model, optimizer, device, epoch, scheduler=None):
    """
    Trains the bert model on the twitter chid_data
    """

...
idiom_vocab = eval(open(data_dir + '/idiomList.txt').readline())  #成语表
print(idiom_vocab)
...
class config:
    ...
    DO_TRAIN = True
    DO_TEST = True
    BERT_PATH = project_dir + "/chinese-roberta-wwm-ext/"
...
class ClozeDataset:
    ...
    def __getitem__(self, item):
        ...
        # print("=======")
        # print(torch.tensor(self.data_id[item], dtype=torch.long).shape)
...
def read_data(path):
    ...
    with open(path, "r", encoding="utf-8") as fin:
        ...
        # for line in fin.readlines():
            ...
            data_id += 1
            if data_id %50000==0 and data_id!=0:
...
def run_one_step(batch, model, device):
    ...
    # Use ids, masks, and token types as input to the models
...
def train_fn(data_loader, model, optimizer, device, epoch, scheduler=None):
    """
    Trains the bert models on the twitter data
    """
    # Set models to training mode (dropout + sampled batch norm is activated)
    model.train()
    # Set tqdm to add loading screen and set the length
    tk0 = tqdm(data_loader, total=len(data_loader))
    # Train the models on each batch
    # Reset gradients
...
def train():
    """
    Train models for a speciied fold
    """
    df_loader = read_data(os.path.join(data_dir, "train_data.txt"))
    model_config = transformers.BertConfig.from_pretrained(config.BERT_PATH)
    # This is important to set since we want to concatenate the hidden states from the last 2 BERT layers
    model_config.output_hidden_states = True
    # Instantiate our models with `model_config`
    ...
    num_train_steps = int(50000/ config.TRAIN_BATCH_SIZE * config.EPOCHS)
    ...
    for df_train in df_loader:
        # print(type(df_train))
        # print(df_train)
        # for indexs in df_train.index:
        #     print(df_train.loc[indexs].values[0:-1])
        ...
        df_loader_dev = read_data(os.path.join(data_dir, "dev_data.txt"))
        df_dev = next(df_loader_dev)
...
def predict():
    df_loader_test = read_data(os.path.join(data_dir, "test_data.txt"))
    # for df_test in dataloader:
    df_test = next(df_loader_test)
